...
## 어법 규칙 — 산문체 누적 금지 (파일럿 03 에서 발견)
state YAML 은 **스냅샷** 이지 **요약 산문** 이 아니다. 다음 어법을 강제한다:
- 각 bullet 1행, **80자 이하 권장 / 120자 초과 금지**.
- 산문체 묘사 금지 — 사실 단위로 분할:
  - ❌ `"ch12 Scene 1 박 부장 집무실: 복도 끝까지 한 번도 멈추지 않고 걸음. 자기 자리 갈림길에서 평소라면 오른쪽 꺾었으나 이 아침에는 안 꺾음. 손등으로 두 번 두드림."`
  - ✅ `"ch12-1 박 부장 집무실 도착, 갈림길 우회 안 함"`
  - ✅ `"ch12-1 손등 노크 2회"`
- 장면별 행동 시퀀스를 통째로 옮기지 말 것 — 그것은 story/chapters/ 본문의 일이다.
- **직전 챕터 변동 사항만 갱신.** 이전 챕터 state 와 동일하면 다시 적지 말 것 (snapshot diff 원칙).
- 누적 비대 위험 임계: character-states.yaml 단일 파일 ≥ 15 KB 이면 산문체 누적 의심 → validator WARN.
