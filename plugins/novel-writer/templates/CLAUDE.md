...
## 슬래시 커맨드 (16종)
`/init-novel [장르]` · `/write-chapter N` · `/continue-writing` · `/gate-status N` · `/run-gate G? N` · `/override-gate G? <사유>` · `/timeline [필터]` · `/bible-lock` · `/bible-unlock <사유>` · `/state [N]` · `/perplexity <N|파일|--calibrate>` · `/revise-loop N` · `/finalize N` · `/resume` · `/metrics [--cost|--project N]` · `/export [--format epub|pdf|docx]` · `/visualize`
