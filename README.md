...
- **21개 서브에이전트** + **18개 슬래시 커맨드** + **5단계 Gate 검증 파이프라인**
- **30 가지 플롯 아키타입 카탈로그** — `/plot-types` 로 검색·결합·장르별 빈출 조회 (v1.7+)
...
[`docs/TUTORIAL.md`](docs/TUTORIAL.md) — **터미널·git 이 처음인 작가도** 따라할 수 있는 안내. 설치(터미널 여는 법부터)·기획·자료조사·Bible·집필·퇴고 전 단계 + 자주 막히는 지점 + 안전 장치 + 도움 받는 법.
